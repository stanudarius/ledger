...
export default async function TickerPage({
  ...
  if (profile && quote) {
    try {
      ...
      // FMP computes PE from raw data which can be currency-mismatched for ADRs.
      // Always prefer Yahoo's pre-computed trailingPE when available.
      try {
        const yahoo = new YahooProvider();
        const ym = await yahoo.getMetrics(symbol);
        if (ym.peRatio > 0) {
          if (!metrics) metrics = ym;
          else {
            metrics = { ...metrics, peRatio: ym.peRatio };
            if (metrics.eps === 0 && ym.eps > 0) metrics.eps = ym.eps;
          }
        }
      } catch { /* Yahoo unavailable — keep provider metrics */ }
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 grid grid-cols-1 md:grid-cols-4 gap-6">
      ...
      <div className="md:col-span-3 space-y-8">
        ...
        <Suspense fallback={<div className="h-48 bg-paper-alt border border-rule-dashed rounded-sm animate-pulse" />}>
          <MarketIntelligenceSection symbol={symbol} provider={provider} />
        </Suspense>

        <Suspense fallback={<div className="h-48 bg-paper-alt border border-rule-dashed rounded-sm animate-pulse" />}>
          <MarketEventsSection symbol={symbol} provider={provider} />
        </Suspense>

        <Suspense fallback={<div className="h-48 bg-paper-alt border border-rule-dashed rounded-sm animate-pulse" />}>
          <OwnershipSection symbol={symbol} provider={provider} />
        </Suspense>
      </div>
    </div>
  );
