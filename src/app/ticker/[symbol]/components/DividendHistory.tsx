...
interface DividendHistoryProps {
  ...
  symbol: string;
  dividends?: Dividend[];
  /** Current market price from quote. Falls back to peRatio * eps when unavailable. */
  currentPrice?: number;
}

export function DividendHistory({ metrics, symbol, dividends = [], currentPrice }: DividendHistoryProps) {
  ...
  const price = currentPrice ?? (metrics.peRatio > 0 && metrics.eps > 0 ? metrics.peRatio * metrics.eps : 0);
  ...
  return (
    <section className="mb-12">
      <SectionHeading number="05 / Dividends" title="Dividends &amp; Yield" />
              ...
              <tbody>
                {divs.slice(0, 10).map((div, i) => (
                  <tr key={i} className="border-b border-rule-dashed last:border-0 hover:bg-paper transition-colors">
                    ...
                    <td className="py-2 text-right font-serif text-sm font-bold text-ink">${div.amount.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
