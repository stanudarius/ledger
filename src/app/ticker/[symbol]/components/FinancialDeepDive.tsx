  ...
  return (
    <>
      ...
      {metrics && (
        <FinancialRatios
          metrics={metrics}
          latestIncome={income.length > 0
            ? [...income].sort((a, b) => a.period.localeCompare(b.period)).at(-1) ?? null
            : null}
          balance={balance}
        />
      )}
      ...
      {metrics && (
        <DividendHistory metrics={metrics} symbol={symbol} dividends={dividends} currentPrice={currentPrice} />
      )}
    </>
  );
}
