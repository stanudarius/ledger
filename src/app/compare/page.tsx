...
import { AddTickerForm } from "@/components/ledger/AddTickerForm";
import { RemoveButton } from "@/components/ledger/RemoveButton";
import { toggleCompare } from "@/actions/userPreferences";
import type { CompanyProfile, Metrics } from "@/lib/providers";
...
export default async function ComparePage() {
  ...
  const tickersData = results
    ...
    .map((r) => r.value)
    .filter(
      (t): t is { symbol: string; profile: CompanyProfile; metrics: Metrics } =>
        t.profile !== null && t.metrics !== null
    );
    ...
    <div className="min-h-screen p-8 max-w-6xl mx-auto space-y-8">
      <div className="flex justify-between items-end border-b border-rule pb-4">
        ...
        <div className="flex flex-col items-end gap-2">
          <AddTickerForm action={toggleCompare} placeholder="Add ticker to compare" className="relative w-64" />
        </div>
      </div>
              ...
              {tickersData.map((ticker) => (
                <div key={ticker.symbol} className="col-span-1 text-center relative group">
                  ...
                  <div className="absolute top-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    <RemoveButton symbol={ticker.symbol} action={toggleCompare} label="Compare" />
                  </div>
                </div>
              ))}
            </div>
