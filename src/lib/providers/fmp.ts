...
export class FMPProvider implements DataProvider {
  ...
  static computeMetrics(raw: {
    marketCap: number; beta: number; lastDividend: number;
    price: number; revenue: number; netIncome: number; prevRevenue: number;
    equity: number; totalAssets: number; eps: number;
  }): Metrics {
    const { marketCap, beta, lastDividend, price, revenue, netIncome, prevRevenue, equity, totalAssets, eps } = raw;
    return {
      marketCap,
      peRatio: eps > 0 && price > 0 ? price / eps : 0,
